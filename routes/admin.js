...
// Cleanup placeholder bookings (admin only)
router.post('/cleanup-old-placeholders', verifyToken, async (req, res) => {
  try {
    console.log('[Cleanup] Removing placeholder bookings...');
    ...
    console.log(`[Cleanup] Deleted ${result.rowCount} placeholder bookings`);

    res.json({
      success: true,
      message: `Cleaned up ${result.rowCount} placeholder bookings`,
...
// Complete schedule reset - delete ALL bookings (admin only)
router.post('/reset-complete-schedule', verifyToken, async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Complete booking reset successful',
